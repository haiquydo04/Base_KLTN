    ...
    <div>
      ...
      <div className="relative">
        ...
        <input type={show ? 'text' : 'password'} value={value} onChange={onChange} placeholder="••••••••"
          className={`w-full h-11 rounded-full border pl-11 pr-11 text-sm text-gray-700 placeholder-gray-300 focus:outline-none focus:ring-2 focus:border-transparent transition-all bg-white ${error ? 'border-primary-300 focus:ring-primary-200' : 'border-primary-100 focus:ring-primary-200'
            }`}
        />
      ...
      {error && <p className="text-primary-500 text-[11px] mt-1 pl-1">{error}</p>}
      {hint && !error && <p className="text-secondary-400 text-[11px] mt-1 pl-1">{hint}</p>}
    </div>
  );
}
...
function Step3({ newPass, setNewPass, confirmPass, setConfirmPass, onNext, onPrev }) {
  ...
  const valid = newPass.length > 0 && newPass === confirmPass;
